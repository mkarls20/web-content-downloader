...
@patch(
    "youtube_web_downloader.app.app.FlaskForm.validate_on_submit", return_value=True
)  # TODO: Dont mock this. Fix the poest call so it validates true
@patch("youtube_web_downloader.app.app.download_youtube_video")
@patch("youtube_web_downloader.app.app.download_youtube_audio")
def test_download_submit_buttons(
    mock_download_youtube_audio: MagicMock,
    mock_download_youtube_video: MagicMock,
    mock_validate_on_submit: MagicMock,
):
    from youtube_web_downloader.app.app import app
...
@patch(
    "os.environ",
    {"DOWNLOAD_FOLDER_PATH": "./test_downloads", "SECRET_KEY": "test_secret"},
)
...
def test_download_youtube_video(mock_open, mock_pickle, mock_youtube):
    ...
    download_youtube_video(
        url
    )  # Call the download_youtube_video function with the given URL

    # Assert
    mock_youtube.assert_called_once_with(
        url
    )  # Check that the mock YouTube object was called with the URL
    mock_youtube.return_value.streams.get_highest_resolution().download.assert_called_once_with(
        output_path="./test_downloads/video"
    )  # Check that the download function was called with the correct output path
...
def test_download_youtube_audio(
    ...
    mock_youtube.return_value.streams.filter.return_value.first.return_value.download.return_value = (
        "./test_downloads/audio/audio.mp4"
    )
    ...
    mock_youtube.return_value.streams.filter(
        only_audio=True
    ).first().download.assert_called_once_with(output_path="./test_downloads/audio")
    ...
    mock_audio_segment.from_file.return_value.export.assert_called_once_with(
        "./test_downloads/audio/audio.mp3",
        format="mp3",
        tags={"title": "Test", "artist": "Tchoupi", "album": "Unknown"},
    )
    mock_pickle.dump.assert_called_once()
    # Test case 2: DOWNLOAD_FOLDER_PATH environment variable is not set
    # Reset mocks
...
@patch("youtube_web_downloader.app.app.download_youtube_audio")
@patch("youtube_web_downloader.app.app.YouTube")
def test_set_track_info_1(
    mock_youtube: MagicMock, mock_download_youtube_audio: MagicMock
):
    from youtube_web_downloader.app.app import app
    ...
    assert (
        b'<input id="track_name" name="track_name" required size="20" type="text" value="Test">'
        in response.data
    )
    assert (
        b'<input id="artist_name" name="artist_name" required size="20" type="text" value="Tchoupi">'
        in response.data
    )
    mock_download_youtube_audio.assert_not_called()
...
def test_set_track_info_2(
    ...
    app.config["WTF_CSRF_ENABLED"] = (
        False  # TODO: Fix this, looks like only testing-problem
    )
    client = app.test_client()
    print(client._cookies)
    youtube_url = "https://www.youtube.com/watch?v=eFyc1g_6ffs"
    ...
    with app.test_request_context():
        ...
        # response = client.get(f'/set_track_info?url={youtube_url}')
        response = client.post(
            "/set_track_info", data=data, content_type="multipart/form-data"
        )
        print(client._cookies)
    ...
    assert response.status_code == 200
    mock_download_youtube_audio.assert_called_once_with(
        youtube_url, "Test", "Tchoupi", "Tchoupi"
    )
...
def test_previous_downloads(
    mock_load_prev_downloads: MagicMock, render_template: MagicMock
):
    from youtube_web_downloader.app.app import app
    ...
    render_template.assert_called_once_with(
        "previous_downloads.html",
        prev_downloads=["download1", "download2", "download3"],
    )
...
def test_delete(
    ...
    mock_dump: MagicMock,
    mock_load_prev_downloads: MagicMock,
):
    from youtube_web_downloader.app.app import app
    ...
    with app.test_request_context(), patch.dict(
        os.environ, {"DOWNLOAD_FOLDER_PATH": download_folder_path}
    ):
    ...
    # Act
    with app.test_request_context(), patch.dict(
        os.environ, {"DOWNLOAD_FOLDER_PATH": ""}
    ):
        response = client.get(f"/delete?url={url}&file_path={file_path}")
